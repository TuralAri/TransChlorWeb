...
//    private $apiKey; //Inutile pour l'instant mais sera ajoutée plus tard pour plus de sécurité (si besoin)
    ...
    /**
     * Sends already existing files on a specified route from the API
     * @param string $meteoFilePath
    ...
    /**
     * Will extract mechanicalInterventions automaticSpray mechanicalThresholdTemperature and
     * automaticThresholdTemperature from the generated outputFile generated by the api
     * on line 6, 12, 9 and 15 for each of them
     * @param string $filePath
     * @return array
     * @throws \Exception
     */
    ...
    /**
     * Generates a JsonResponse containing
     * 'mechanicalInterventions'
     * 'automaticSprays'
     * 'mechanicalThresholdTemperature' and
     * 'automaticThresholdTemperature'
     * @param string $filePath
     * @return JsonResponse
     * @throws \Exception
     */
    ...
    /**
     * Will compute the missing 4 values from the exposure-series form by sending the form's data and meteoFile to the
     * API
     * @param array $data exposure-series form's data
     * @param string $meteoFileName name of the used meteoFile (from WeatherStation)
     * @return JsonResponse
     */
    ...
    /**
     * Will send data and meteoFileName to the API in order to generate a zip containing all exposures
     * @param array $data Filled exposure-series form's data
     * @param string $meteoFileName Name of the used MeteoFile (from WeatherStation)
     * @return Response Success or not (if success zip file containing all exposure files will be inside)
     */
     ...
     * @throws \Symfony\Contracts\HttpClient\Exception\TransportExceptionInterface
     */
    public function start1DComputing(int $computationId, string $inputFile, $data) :void
    {
        $this->httpClient->request('POST', $this->apiUrl . '/api/computing/run', [
            'json' => [
                'mode' => '1D',
